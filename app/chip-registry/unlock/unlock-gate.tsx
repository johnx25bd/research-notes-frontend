...
const DEMO_PATH = "/demos/chip-registry"

// Lucid Computing design-system tokens, matching the demo behind this gate
...
export function UnlockGate() {
  ...
  async function onSubmit(e: FormEvent) {
    ...
    try {
      const res = await fetch("/api/chip-registry-unlock", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ password }),
      })
      if (res.ok) {
        // Cookie is set; reload into the demo (the proxy now lets it through).
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Chip registry &mdash; declared compute
        </h1>
        <p style={{ margin: "0 0 26px", fontSize: "15px", color: MUTED, lineHeight: 1.55 }}>
          This demo is passphrase-protected. Enter the passphrase to view it.
        </p>

        <form onSubmit={onSubmit} noValidate>
          <label
            htmlFor="chip-registry-pass"
            style={{
              display: "block",
              fontSize: "13px",
              fontWeight: 600,
          ...
          <div style={{ minHeight: "20px", marginTop: "8px" }}>
            ...
            {status === "unavailable" && (
              <p style={{ margin: 0, fontSize: "13px", color: ERROR }} role="alert">
                This demo isn&rsquo;t available yet. Get in touch if you were expecting access.
              </p>
            )}
          </div>
          ...
          >
            {status === "submitting" ? "Checking…" : "View demo"}
          </button>
          <p
            style={{
              margin: "18px 0 0",
          ...
          >
            <span style={{ color: AMBER_INK }}>Lucid Computing</span> &middot; pre-release demo
          </p>
        </form>
      </div>
    </main>
