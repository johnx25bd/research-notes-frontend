...
export async function POST(req: NextRequest) {
  ...
  // Fail closed. With no CHIP_REGISTRY_PASSWORD configured there is no
  // passphrase to match, so no request can unlock the demo.
  ...
  res.cookies.set(CHIP_REGISTRY_COOKIE, token, {
    httpOnly: true,
    secure: true,
    sameSite: "lax",
    path: CHIP_REGISTRY_PATH,
